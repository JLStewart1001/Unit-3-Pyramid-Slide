...
formElem.onsubmit = function(event) {
    ...
    // it wont prevent the default functions from happening.
    event.preventDefault();

    // QUIZ
    // what happens if we don't do this?
    // the errors wont clear after.
    clearError();
    ...
    var tooShort = 0;
    if (height < tooShort) {
        displayError("Too low of a number.");
        return;
    }
...
function clearError(message) {
  document.querySelector(".error-message").innerHTML = clearMessage;
}
